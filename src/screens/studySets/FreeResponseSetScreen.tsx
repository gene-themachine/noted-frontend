...
export default function FreeResponseSetScreen() {
  ...
  // Get unique library items from the free response set
  const freeResponseLibraryItems = useMemo(() => {
    return freeResponseSet?.libraryItems || [];
  }, [freeResponseSet]);
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="flex items-center gap-3 text-foreground-secondary">
          ...
          Loading free response set...
        </div>
      </div>
    );
  }

  if (error || !freeResponseSet) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-foreground-secondary">
        <div className="text-lg mb-4">Error loading free response set</div>
        <div className="text-sm mb-6">The free response set you're looking for doesn't exist or may have been deleted.</div>
        <button 
          onClick={handleBack}
  ...
  const handleStartPractice = () => {
    navigate(`/project/${projectId}/study-sets/free-response/${setId}/study`);
  };
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-normal text-gray-500">
                ({freeResponseLibraryItems.length})
              </span>
            </div>
            <div className="flex items-center gap-2">
              {showAttachedFiles ? (
            ...
            {showAttachedFiles && (
              ...
              >
                <div className="flex flex-wrap gap-2 p-2 bg-gray-50 rounded-lg">
                  {freeResponseLibraryItems.map((item) => (
                    <div
                      key={item.id}
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {questions.map((question: any, index: number) => (
              <div
                key={question.id}
                className="group bg-white border border-gray-200 rounded-2xl p-4 hover:border-green-500 hover:shadow-sm transition-all duration-200 cursor-pointer"
                onClick={() => navigate(`/project/${projectId}/study-sets/free-response/${setId}/study/${question.id}`)}
              >
